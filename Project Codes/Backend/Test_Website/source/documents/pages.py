...
class PdfDocumentTest:
    ...
    def is_valid_format(self):
        dimensions, justified, fonts, margins = True, True, True, True
        for page_no in range(self.page_count):
            page = self.document.loadPage(page_no)
            dimensions = self.check_page_dimensions(page_no, page.bound()) and dimensions
            data = page.getText('dict')
            block_no = 1
            for block in data['blocks']:
                if block['type'] != 1:
                    justified = self.is_justified(page_no + 1, block_no, block['lines']) and justified
                    fonts = self.check_fonts(page_no, block_no, block['lines']) and fonts
